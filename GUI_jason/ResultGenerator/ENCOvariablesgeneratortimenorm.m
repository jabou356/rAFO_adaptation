function [AnalENCO]=ENCOvariablesgeneratortimenorm(Cycle_Table, data, conditions, criticalCycles, SyncData)
    ...
    for icond = 1 : length(conditions)
        ...
        for istride=criticalCycles(icond,isubject)+1:criticalCycles(icond+1,isubject) % there is probably a way to vectorize this
            ...
            if Cycle_Table{isubject}(3,istride)==1 && SyncData.SyncTiming{isubject}(istride) < length(data{isubject}{istride})
                ...
                AnalENCO.ENCO.(conditions{icond}){isubject}(:,k)=interp1(x,y,1:(length(x)-1)/(999):length(x));
        ...
        if icond == 1
            ...
            if length(novalid) > 5
                ...
                tempcycles = find(~isnan(AnalENCO.ENCO.(conditions{icond}){isubject}(:,BASELINEend-49:BASELINEend)));
